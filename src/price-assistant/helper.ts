...
export function preparePriceFeedConfig(
  walletConfig: WalletManagerConfig,
  network?: string,
) {
  const priceFeedSupportedTokens = Object.values(walletConfig).reduce(
    (acc, chainConfig) => {
  ...
  const environment: Environment = network
    ? (network as Environment)
    : Environment.MAINNET;
  const nativeTokens = supportedNativeTokensByEnv[environment];

  for (const nativeToken of nativeTokens) {
    const isNativeTokenDefined = priceFeedSupportedTokens.find(
      token => token.coingeckoId === nativeToken.coingeckoId,
    );
