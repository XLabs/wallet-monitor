...
export class SuiWalletToolbox extends WalletToolbox {
  ...
  public async pullNativeBalance(
    ...
    if (blockHeight) {
      this.logger.warn(
        `Sui does not support pulling balances by block height, ignoring blockHeight: ${blockHeight}`,
      );
    }
  ...
  public async pullTokenBalances(
    ...
    return uniqueTokens.map(tokenAddress => {
      ...
      const balance = allBalances.find(
        balance => balance.coinType === tokenData.address,
      );
      if (!balance) {
        return {
          tokenAddress,
      ...
      const formattedBalance = formatFixed(balance.totalBalance, tokenDecimals);
  ...
  public async getBlockHeight(): Promise<number> {
    ...
    const sequenceNumber =
      await suiJsonProvider.getLatestCheckpointSequenceNumber();
