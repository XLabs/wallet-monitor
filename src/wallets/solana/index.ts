...
export class SolanaWalletToolbox extends WalletToolbox {
  ...
  constructor(
    ...
    if (!this.options.nodeUrls)
      throw new Error(`No default node url provided.`);

    // TODO figure out a fallback provider-like mechanism for solana
    this.options.nodeUrls = Array.isArray(this.options.nodeUrls)
      ? this.options.nodeUrls
      : [this.options.nodeUrls];
    this.connection = new Connection(this.options.nodeUrls[0]);
  }
